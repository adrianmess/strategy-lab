...
- UI: Max DD (`gmChips_dd`), Max days in trade (`gmChips_mh`), Evals per run (`gmChips_total`) are chip editors (`GM_LISTS` state, `gmAdd/gmDel/gmRender`, localStorage key `gmLists`). Run count/time estimate account for all budgets (`sumT/40000` min heuristic).

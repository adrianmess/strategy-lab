...
def main():
    ...
    _using_pt_file = False
    if os.path.abspath(args.space) == os.path.abspath(_default_space) \
            and os.path.exists(_pt_space):
        args.space = _pt_space
        _using_pt_file = True
        print(f"parameter space file: param_spaces/"
              f"{os.path.basename(_pt_space)} (pair+timeframe default)",
              flush=True)
    ...
    if args.symbol.lower() != "sol" and not _using_pt_file:
        _ovp = os.path.join(B.OPT_DIR, "param_spaces",
                            f"{args.symbol.lower()}.json")
        if os.path.exists(_ovp):
            _ov = json.load(open(_ovp))
            _fam = ((_ov.get(f"{args.strategy}@spot") if args.mode == "spot"
                     else None) or _ov.get(args.strategy) or {})
            import copy as _copy
            space = _copy.deepcopy(space)
            n_ov = 0
            for _k, _rng in _fam.items():
                if _k in (space.get("continuous") or {}):
                    space["continuous"][_k] = dict(space["continuous"][_k],
                                                   range=list(_rng))
                    n_ov += 1
            print(f"pair overlay: {n_ov} ranges scaled for "
                  f"{args.symbol.upper()} ({_ovp})", flush=True)
        else:

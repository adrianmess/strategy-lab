...
def pairs_specs():
    """Campaign c7 — per-pair SPOT searches for the multi-pair basket.
    Each pair runs the proven families on ITS OWN spot candles with
    vol-scaled parameter ranges (param_spaces/<coin>.json overlays,
    generated by gen_pair_spaces.py from chart history + Adrian's BTC/ETH
    reference configs). SOL is excluded (already has champions)."""
    S = []
    for coin in ("btc", "eth", "doge", "xrp", "sui"):
        for fam in ("v7", "prime7", "macdx", "rocx"):
            S.append(dict(id=f"p_{coin}_{fam}", wave=1, strategy=fam,
                          mode="spot", method="vol3", space="default",
                          algo="genetic", scoring="classic", total=150000,
                          mh=7, hold=21, symbol=coin, status="pending"))
    return S
...
class Campaign:
    ...
    def gen_wave2(self):
        ...
        groups = {}
        for g, s in scored:
            # symbol in the key: NEVER breed-merge runs from different pairs
            # (different price data — incompatible pools)
            groups.setdefault((s["strategy"], s["mode"], s["method"],
                               s.get("symbol")), []).append(s)
        era = self.plan.get("matrix") in ("era", "survival")
        for (strat, mode, meth, sym), members in groups.items():
            if len(members) >= 2:
                rf = ",".join(f"runs/{run_name(self.name, m)}" for m in members[:4])
                new.append(dict(id=f"w2_merge_{strat}_{mode}_{meth}"
                                   + (f"_{sym}" if sym else ""), wave=2,

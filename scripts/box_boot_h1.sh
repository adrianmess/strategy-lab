...
tmux has-session -t h1 2>/dev/null || tmux new-session -d -s h1 \
  ". ~/venv/bin/activate && cd ~/strategy-lab/optimizer && python3 gamut_worker.py --plan campaigns/hyp1a_0811/plan.json --jobs $J $REV 2>&1 | tee -a ~/worker_h1.log"

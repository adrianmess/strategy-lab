...
if ! pgrep -xf "python3 gamut_worker.py --plan campaigns/gamut_g0801_2122/plan.json --jobs 17" >/dev/null; then
  tmux kill-session -t gamut 2>/dev/null
  tmux new-session -d -s gamut '. ~/venv/bin/activate && cd ~/strategy-lab/optimizer && python3 gamut_worker.py --plan campaigns/gamut_g0801_2122/plan.json --jobs 17 2>&1 | tee -a ~/worker.log'
fi
tmux has-session -t hype 2>/dev/null || tmux new-session -d -s hype 'while pgrep -xf "python3 gamut_worker.py --plan campaigns/gamut_g0801_2122/plan.json --jobs 17" >/dev/null; do sleep 120; done; . ~/venv/bin/activate && cd ~/strategy-lab/optimizer && python3 gamut_worker.py --plan campaigns/gamut_ghype/plan.json --jobs 13 2>&1 | tee -a ~/worker_hype.log'

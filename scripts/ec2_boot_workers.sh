...
if ! pgrep -xf "python3 gamut_worker.py --plan campaigns/gamut_g0801_2122/plan.json --jobs 13" >/dev/null; then
  ...
  tmux new-session -d -s gamut '. ~/venv/bin/activate && cd ~/strategy-lab/optimizer && python3 gamut_worker.py --plan campaigns/gamut_g0801_2122/plan.json --jobs 13 2>&1 | tee -a ~/worker.log'
fi
